...
void test_fork()
{
    ...
    const auto child_process = []() {
        TRACE_SCOPE_CAT("child_process", "fork");
        std::println("Child process running: {}", getpid());
    };
    const auto parent_process = [pid]() {
        TRACE_SCOPE_CAT("parent_process", "fork");
...
{
    const char* pipe_path = (argc < 2) ? "/tmp/tracer-trace_file_test.json" : argv[2];

    std::println("Starting profiler test...\n");
    TRACE_SETUP(pipe_path);
